...
guard let assets = try? fm.contentsOfDirectory(at: root, includingPropertiesForKeys: nil)
        .first(where: { $0.pathExtension == "" && fm.fileExists(atPath: $0.appendingPathComponent("Resources/Assets.xcassets").path) })
        .map({ $0.appendingPathComponent("Resources/Assets.xcassets/AppIcon.appiconset") }) else {
